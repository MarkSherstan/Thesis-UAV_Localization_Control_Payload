...
yawData = yawData[0:100]
imuData = yawData.copy()

bigData = np.array([np.array([yawData, imuData]).T])
...
# Real time sim
for ii in range(bigData.shape[1]):
    raw = bigData[0][ii]
    dataLog.append(KF.update(raw))
...
##########################
# t = range(data.size)
plt.plot(range(yawData.size), yawData, 'k-', alpha=0.2, label='Raw Data')
...
plt.plot(range(bigData.shape[1]), dataLog, 'k-', label='Fused')
