...
for ii in range(3, 30):
    adaptiveThreshWinSizeMin  = ii
    adaptiveThreshWinSizeMax  = ii
    adaptiveThreshWinSizeStep = ii
    ...
    if (min3 and maxMin and not0):
        ...
        if (nScales == 1):
            ...
            # Thresh windows
            thresh = adaptiveThreshWinSizeMin
                                
            # Log the data
            data.append([adaptiveThreshWinSizeMin, adaptiveThreshWinSizeMax,
                            adaptiveThreshWinSizeStep, countA, countB, countA+countB, 
                            thresh, len(camA), len(camB)])

            # Update user
            print('{:<d} of 27 complete. Time elapsed: {:<0.2f}'.format(counter, time.time()-startTime))
            counter += 1
